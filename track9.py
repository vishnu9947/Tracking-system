...
def recognize_faces(frame, known_face_embeddings, known_names, roi_points):
    ...
    for face in faces:
        ...
        if name == "Unknown":
            # Unknown faces get a red bounding box, regardless of ROI
            cv2.rectangle(frame, (left, top), (right, bottom), (0, 0, 255), 2)
        else:
            if is_face_in_roi(bbox_center, roi_points):
                # Known face inside ROI gets a green bounding box
                cv2.rectangle(frame, (left, top), (right, bottom), (0, 255, 0), 2)
            else:
                # Known face outside ROI gets a yellow bounding box
                cv2.rectangle(frame, (left, top), (right, bottom), (0, 255, 255), 2)

        # Display the name below the bounding box (if recognized)
