...
# Load camera data from MongoDB
camera_data = load_camera_data()

# Use the first camera link for the video capture
first_camera_id, camera_info = list(camera_data.items())[0]
video_capture = cv2.VideoCapture(camera_info["stream_link"])

# Start processing video with dynamic frame skipping
process_video_dynamic_skipping(video_capture, known_face_embeddings, known_names, employee_id_mapping, camera_info["rois"])
