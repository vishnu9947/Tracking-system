...
def log_entry(employee_id, roi_id):
    """
    Log entry time for the recognized person along with ROI ID.
    """
    ...
    data = {
        ...
        "roi_id": roi_id  # Store the ROI ID in the database
    }
    try:
...
def log_exit(employee_id, last_seen_time, roi_id):
    """
    Log exit time for the recognized person along with ROI ID.
    """
...
def process_video_dynamic_skipping(video_capture, known_face_embeddings, known_names, rois):
...
# Run face recognition on the live video stream with dynamic frame skipping
process_video_dynamic_skipping(video_capture, known_face_embeddings, known_names, ROIS)
